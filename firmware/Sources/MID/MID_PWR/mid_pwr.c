...
/**
 * @fn static int16_t _divide_dec(const int16_t dP, const uint16_t mV)
 * @brief Makes the division between a number in dU and mU and return the answer in mU
 * @param dP, Value of the power in dW.
 * @param V, Value of the voltage in mV
 * @return int16_t the value of the current in mA
 */
									 ...
									 const MID_REG_limit_s limits, int16_t *action_res)
{
	...
	{ // out for pwr or v PI is allways current
	...
	else if (mode == MID_PWR_MODE_CC)
	{ // for i PI actions is the Duty to apply
...
MID_PWR_result_e MID_PwrApplyCtrl(const int16_t ref, const uint16_t V_LS, const int16_t I_LS, const MID_PWR_Mode_e control_mode, const MID_REG_limit_s limits)
{
	...
	int16_t new_duty;
	switch (control_mode)
	{ // first PI, if needed (in Pwr and V refs)
	...
	if (res == MID_PWR_RESULT_SUCCESS)
	{
		...
		{
			curr_ref = _divide_dec(limits.lsPwrMax, V_LS);
		}
		else if (SOA_status == SOA_under_pwr)
		{
			curr_ref = _divide_dec(limits.lsPwrMin, V_LS);
		}
